...
状態が `G2` で G2g の明示集合に含まれる ID は、担当する正典文書の規則注釈と G2g テストの両方で参照されなければならない。
...
- **状態**：G5
- **由来**：ホワイトペーパー §11.5.8
- **内容**：仮引数の型、結果の型、捕捉の型に借用型を許す。region 欄が束縛された `(RParam rp)` でなければ違反とする。

### OWN-005 計算した値の place 化
...
### OWN-007 `Owned` を捕捉する closure

- **状態**：G5
- **由来**：ホワイトペーパー §4.7
- **内容**：`Lam` は外側の `Owned<τ>` を捕捉できる。捕捉は `Curry` の固定引数として符号化し、捕捉した closure の型は `Owned<NFn …>` である。関数の位置へ置くには `Move` を経由する
...
たとえば `proof-search.md` §7 は太字の項目名を持たないため、本表で名前を与えている。
