...
`message` は診断ごとの説明であり、現行の `diagnostic-of` は `title` と同じ文字列を入れる。
複数の文からなる文案を入れるのは後続 Phase の仕事である。
...
`diagnostic-schema-version` は 4 であり、`diagnostic-registry-version` は 21 である。
...
`diagnostic-fixture-v1.rkt` から `diagnostic-fixture-v22.rkt` まで、registry version ごとに 1 本を置く。

組数は v1 から順に 59、107、120、132、136、136、138、139、141、153、155、156、162、169、180、185、186、187、193、194、195、198 である。
...
v11 は elaborate へ `E-OWN-029` を、typing へ `E-OWN-028` を 1 行ずつ足した。

どちらも key は `owned-narrowing-rejected` であり、registry の鍵が `(phase key)` であるために phase ごとに別の code を持つ。

廃止した行も registry に残るため、組は 153 に 2 を足した 155 になる。

v12 は elaborate へ `E-SYN-005` を 1 行足した。

v13 は typing へ `E-VAR-008`、`E-VAR-009`、`E-VAR-010` を、elaborate へ `E-VAR-011`、`E-VAR-012`、`E-VAR-013` を 3 行ずつ足した。
...
v16 は typing へ `E-OWN-030` と `E-PRF-009` から `E-PRF-011` までの 4 行を、elaborate へ `E-OWN-031` を足した。
...
v18 は typing へ `E-ORG-002` の 1 行を足した。

廃止した行は無いため、組は 186 に 1 を足した 187 になる。
...
v21 は surface へ `E-SUR-020` を 1 行足した。
`E-SUR-020` は「型位置の &、または trait の要求型の Self を対象型で置き換えた結果が正規化できない」という診断である。
廃止した行は無いため、組は 194 に 1 を足した 195 になる。

v22 は surface へ `E-SUR-021` から `E-SUR-023` までの 3 行を足した。
廃止した行は無いため、組は 195 に 3 を足した 198 になる。
