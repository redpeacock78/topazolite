...
;; OWN-004。構造型 narrowing が余剰 Owned field を失う場合の拒否である。
;; 同じ key を elaborate 側も使うが、registry の鍵は (phase key) であるため
;; phase ごとに別の code を割り当てる。E-TYP-012 と E-TYP-023 と同じ形である。
(define typing-entries-v11
  '(("E-OWN-028" owned-narrowing-rejected
                 "構造型 narrowing が余剰 Owned field を失う")))

(define elaborate-entries-v11
...
;; P1c2b。mut binding と Reassign の 3 つの key を phase ごとに別の code
;; で持つ。分類内番号は初回割当なので key 記号の辞書順である。
(define typing-entries-v13
  '(("E-VAR-008" immutable-binding
                 "mut でない binding への再代入")
    ("E-VAR-009" mut-binding-unsupported-type
                 "mut binding の型に Owned または借用が含まれる")
    ("E-VAR-010" reassign-type-mismatch
                 "再代入する値の型が slot の型と一致しない")))

(define elaborate-entries-v13
  '(("E-VAR-011" immutable-binding
                 "mut でない binding への再代入")
    ("E-VAR-012" mut-binding-unsupported-type
                 "mut binding の型に Owned または借用が含まれる")
...
(define surface-entries-v22
  '(("E-SUR-021" surface-trait-in-type-position "trait 名を型の位置で使った")
