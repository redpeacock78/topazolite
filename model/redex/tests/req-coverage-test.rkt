...
(test-case "main resolves repository paths and reports the expected count"
  ...
  (check-equal? (get-output-string output)
                (string-append
                 ...
                 "16 G5 IDs, 1 P1b IDs, 1 P1c1 IDs, 1 P1c2b IDs, 1 P1d IDs, "
                 "1 P1e3 IDs, 1 P2c IDs, 1 P2d IDs, 1 P2e1 IDs, 1 P2e2 IDs, 1 P2f IDs, 1 P2g IDs, 1 P2h1 IDs, 2 P2h2 IDs, 1 P2h3a IDs, 1 P2h3b IDs\n"
...
(test-case "cycle-descriptors covers every declared sub-cycle"
  ...
  (check-equal? (map cycle-descriptor-name ds)
                '(G1 G2a G2b G2c G2d G2e G2f G2g G3a G3b G3c G3d G4 G5 P1b P1c1 P1c2b P1d P1e3 P2c P2d P2e1 P2e2 P2f P2g P2h1 P2h2 P2h3a P2h3b))
  (define (by-name n)
  ...
  (check-equal? (cycle-descriptor-expected-ids (by-name 'G3b)) '(BAK-001))
  (check-false (cycle-descriptor-expected-count (by-name 'G3b)))
  ...
  (check-equal? (cycle-descriptor-expected-ids (by-name 'G4))
  ...
  (check-equal? (cycle-descriptor-expected-ids (by-name 'P1b)) '(OWN-004))
  (check-false (cycle-descriptor-expected-count (by-name 'P1b)))
  (check-equal? (cycle-descriptor-expected-ids (by-name 'P1c1)) '(SCP-002))
  (check-false (cycle-descriptor-expected-count (by-name 'P1c1)))
  (check-equal? (cycle-descriptor-expected-ids (by-name 'P1c2b)) '(SCP-001))
  (check-false (cycle-descriptor-expected-count (by-name 'P1c2b)))
  (check-equal? (cycle-descriptor-expected-ids (by-name 'P1d)) '(NAR-003))
  (check-equal? (cycle-descriptor-expected-ids (by-name 'P1e3)) '(MAC-001))
  (check-equal? (cycle-descriptor-expected-ids (by-name 'P2c)) '(SUR-001))
  (check-equal? (cycle-descriptor-expected-ids (by-name 'P2f)) '(NAR-005))
  (check-false (cycle-descriptor-expected-count (by-name 'P2f)))
  (check-equal? (cycle-descriptor-state (by-name 'P1d)) "P1")
  (check-equal? (cycle-descriptor-state (by-name 'P1e3)) "P1")
  (check-equal? (cycle-descriptor-state (by-name 'P2c)) "P2")
  (check-equal? (cycle-descriptor-state (by-name 'P1c2b)) "P1")
  (check-equal? (cycle-descriptor-state (by-name 'P1c1)) "P1"))
...
(test-case
 "descriptor state outside the started cycles is rejected"
 (define errors
   (with-fixture
    (registry-entry bak-003 "G3")
    (format "[REQ: ~a]" bak-003)
