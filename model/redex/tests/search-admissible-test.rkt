...
(define PT/span '(ProofRep (#:span #:synthetic 0 0)
                           (Reserved o-type-narrative)
                           TypeNarrativeCap))
(define PT2/span '(ProofRep (#:span #:synthetic 0 0)
                            (Reserved o-type-narrative-b)
                            TypeNarrativeCap))
...
(check-true  (admissible? goalT Γ-pc0 'Finite (resolved PT/span) (sigma-for goalT)))
; 一意性導出はあるが SR の P が Σ 由来の P と食い違う → 却下
(check-false (admissible? goalT Γ-pc0 'Finite (resolved PT2/span) (sigma-for goalT)))
; 完全でない Σ（空）では一意性導出が無い → 却下
(check-false (admissible? goalT Γ-pc0 'Finite (resolved PT/span) '()))
...
(check-false (admissible? goalT gamma-pc2 'Finite (resolved PT/span) incomplete-sigma))
; 一意性証拠が無い → 却下
(check-false (admissible? goalT Γ-pc0 'Finite (resolved PT/span) #f))
; (Finite, Absent) → 却下
