...
(test-case
 ...
 ;; 足し忘れや二重登録が通ってしまう。
 (check-equal? (length diagnostic-registry) 193)
 ...
 (check-equal? (count-of 'surface) 18)
 ...
 ;; elaborate / typing、version 14 の expand、version 15、17、19 の surface、
...
;; test 12
(test-case
 "schema version は 4、registry version は 19 である"
 (check-equal? diagnostic-schema-version 4)
 (check-equal? diagnostic-registry-version 19))

(test-case
 "typing の registry version 19 と入口 key"
 (check-equal? diagnostic-registry-version 19)
 (check-equal? (diagnostic-code-of 'typing 'ill-typed) "E-TYP-001")
...
(test-case
 "凍結 fixture v11 の全 (code phase key) が現在の registry に同じ組である"
 (check-equal? (length diagnostic-entries-v11) 155)
 (for ([entry (in-list diagnostic-entries-v11)])
   (match-define (list code phase key) entry)
   (define row (diagnostic-code-row code))
   (check-true (and row
                    (eq? (diagnostic-code-phase row) phase)
...
(test-case
 "凍結 fixture v12 の全 (code phase key) が現在の registry に同じ組である"
 (check-equal? (length diagnostic-entries-v12) 156)
 (for ([entry (in-list diagnostic-entries-v12)])
   (match-define (list code phase key) entry)
   (define row (diagnostic-code-row code))
   (check-true (and row
                    (eq? (diagnostic-code-phase row) phase)
...
(test-case
 "凍結 fixture v14 の全 (code phase key) が現在の registry に同じ組である"
 (check-equal? (length diagnostic-entries-v14) 169)
 (for ([entry (in-list diagnostic-entries-v14)])
   (match-define (list code phase key) entry)
   (define row (diagnostic-code-row code))
   (check-true (and row
                    (eq? (diagnostic-code-phase row) phase)
...
(test-case
 "凍結 fixture v15 の全 (code phase key) が現在の registry に同じ組である"
 (check-equal? (length diagnostic-entries-v15) 180)
 (for ([entry (in-list diagnostic-entries-v15)])
   (match-define (list code phase key) entry)
   (define row (diagnostic-code-row code))
   (check-true
    (and row
...
(test-case
 "凍結 fixture v16 の全 (code phase key) が現在の registry に同じ組で存在する"
 (check-equal? (length diagnostic-entries-v16) 185)
 (for ([entry (in-list diagnostic-entries-v16)])
   (match-define (list code phase key) entry)
   (define row (diagnostic-code-row code))
   (check-true
    (and row
...
(test-case
 "凍結 fixture v19 の全 (code phase key) が現在の registry に同じ組で存在する"
 (check-equal? (length diagnostic-entries-v19) 193)
 (for ([entry (in-list diagnostic-entries-v19)])
   (match-define (list code phase key) entry)
   (define row (diagnostic-code-row code))
   (check-true
    (and row
...
(test-case
 "narrowing の拒否は phase ごとに別の code を持つ"
 (check-equal? (diagnostic-code-of 'typing 'owned-narrowing-rejected)
               "E-OWN-028")
