...
(test-case
 ...
 ;; 足し忘れや二重登録が通ってしまう。
 (check-equal? (length diagnostic-registry) 185)
 ...
 (check-equal? (count-of 'surface) 11)
 ...
 ;; elaborate / typing、version 14 の expand、version 15 の surface である。
...
;; test 12
(test-case
 "schema version は 4、registry version は 16 である"
 (check-equal? diagnostic-schema-version 4)
 (check-equal? diagnostic-registry-version 16))

(test-case
 "typing の registry version 16 と入口 key"
 (check-equal? diagnostic-registry-version 16)
 (check-equal? (diagnostic-code-of 'typing 'ill-typed) "E-TYP-001")
